...
var game_colors = initial.colors.slice(0);
...
document.addEventListener('mouseover', mouse_move, false);
...
var delay = 10 // 10ms delay between frames
...
function click_buttons(evt)
{
    if(mouse.down == 0 && mouse.prev_down == 1)
        {
            buttons_ids = Object.keys(buttons);
    ...
    if(fixed)
        {
            ...
            canvas.style.left = window.innWidth / 2 - canvas.width / 2
...
function draw_button(x, y, content, gap = 10, text_size = 30, border_radius = 10, border_thickness = 3, font = "Comic Sans MS", fill = '#eee', text_color = 'red', fill_hover = 'black', text_hover = '#ddd', fill_down = '#aaa', text_down = '#bbb', border = 'black', text_border = 'red')
{
    ...
    if(mouse.x >= x - box_width / 2 - gap && mouse.x <= x - box_width / 2 + box_width + gap && mouse.y >= y && mouse.y <= y + text_size + gap)
        {
            fill = fill_hover
    ...
    ctx.fillStyle = fill;
    fillRoundRect(x - box_width / 2 - gap, y, box_width + gap * 2, text_size + gap, border_radius)
    ctx,fillStyle = border
    roundRect(x - box_width / 2 - gap, y, box_width + gap * 2, text_size + gap, border_radius, border_thickness)
